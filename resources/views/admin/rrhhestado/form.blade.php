<div class="box box-info padding-1">
    ...
    <div class="box-body">
        ...
        <div class="form-group">
            {{ Form::label('nombre') }}
            {{ Form::text('nombre', $rrhhestado->nombre, ['class' => 'form-control' . ($errors->has('nombre') ? ' is-invalid' : ''), 'placeholder' => 'Nombre Estado',"oninput"=>"this.value = this.value.toUpperCase()"]) }}
            {!! $errors->first('nombre', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('nombre corto') }}
            {{ Form::text('nombre_corto', $rrhhestado->nombre_corto, ['class' => 'form-control' . ($errors->has('nombre_corto') ? ' is-invalid' : ''), 'placeholder' => 'Nombre Corto',"oninput"=>"this.value = this.value.toUpperCase()"]) }}
            {!! $errors->first('nombre_corto', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('factor') }}
            {{ Form::number('factor', $rrhhestado->factor, ['class' => 'form-control' . ($errors->has('factor') ? ' is-invalid' : ''), 'placeholder' => '0.00', 'step' => 'any']) }}
            {!! $errors->first('factor', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('color') }}
            {{ Form::hidden('color', $rrhhestado->color, ['class' => 'form-control' . ($errors->has('color') ? ' is-invalid' : ''), 'id' => 'colorInput']) }}
    ...
    <script>
        ...
        preview.style.backgroundColor = "{{$rrhhestado->color?$rrhhestado->color:'#FFF'}}";
